...
<img width="1272" height="560" alt="image" src="https://github.com/user-attachments/assets/8ffa1158-3b96-4d5b-9c78-c691586bcda8" />
...
<img width="1186" height="502" alt="image" src="https://github.com/user-attachments/assets/ae05a3e7-9aa1-455b-99ba-039fd07166a5" />
...
The Common Source (CS) amplifier designed using an NMOS amplifying transistor (M1), a PMOS active load (M2), and an NMOS current source (M3) in **TSMC 180 nm CMOS technology** was successfully implemented and verified in LTSpice under the specified constraints.

The circuit operates with a supply voltage of **$V_{DD} = 1.8 \text{ V}$** while satisfying the strict power requirement:

$$
P \leq 1 \text{ mW}
$$

By selecting a drain current of approximately **$I_D = 200 \,\mu A$**, the total power dissipation is:

$$
P = V_{DD} \times I_D = 1.8 \times 200\,\mu A = 0.36 \text{ mW}
$$

which is well below the maximum allowable limit. This confirms that the design achieves low-power operation while maintaining proper amplification characteristics.

DC operating point analysis verifies that all three transistors operate in the **saturation region**, ensuring valid small-signal operation. The biasing strategy successfully positions the output node near the mid-supply region, providing sufficient voltage headroom for symmetrical signal swing and preventing distortion due to cutoff or triode operation.

The PMOS active load significantly increases the effective output resistance compared to a passive resistor implementation, enabling higher gain within a compact CMOS-compatible structure. The NMOS current source provides stable biasing and improves robustness against supply and process variations. Although its finite output resistance introduces source degeneration that reduces gain, it enhances linearity and bias stability.

Simulation results confirm proper amplifier behavior:

- The circuit demonstrates clear **180° phase inversion**, characteristic of a CS amplifier.  
- The measured midband voltage gain is approximately **$11–12 \text{ dB} \,(\approx 3.8 \,V/V)$**.  
- The −3 dB bandwidth is approximately **$f_{-3dB} \approx 67.8 \text{ kHz}$**, indicating dominant pole behavior consistent with intrinsic MOSFET parasitic capacitances.  
- The frequency response exhibits classical **low-pass characteristics**, with gain roll-off at higher frequencies due to $C_{gs}$, $C_{gd}$, and junction capacitances.

Differences between ideal theoretical predictions and simulated results arise from practical second-order effects included in the device models, such as channel length modulation, mobility degradation, body effect, and parasitic capacitances.

Overall, the designed CS amplifier satisfies the imposed voltage and power constraints while achieving stable biasing, reliable saturation operation, controlled voltage gain, and predictable frequency response. The implementation effectively demonstrates key CMOS analog design principles, including active loading, current-source biasing, and small-signal frequency analysis, making it suitable for low-power analog signal amplification applications.
